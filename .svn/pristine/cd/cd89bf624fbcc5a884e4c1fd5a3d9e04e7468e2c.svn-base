...
const BeneficiaryPersonalDetails = ({
  ...
  const [selectedDistrictNfbs, setSelectedDistrictNfbs] = useState('');
  const [selectedAreaNfbs, setSelectedAreaNfbs] = useState('');
  const [selectedSubDistrictNfbs, setSelectedSubDistrictNfbs] = useState('');
  const [selectedGramPanchayatWardNfbs, setSelectedGramPanchayatWardNfbs] = useState('');
  const [selectedVillageNfbs, setSelectedVillageNfbs] = useState('');
  const [selectedHabitationNfbs, setSelectedHabitationNfbs] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('application no navigate back', formData.applicationNo);
        if (formData.applicationNo) {
          //const parsedData = JSON.parse(storedData);

          const beneficiaryResponse = await axiosInstance.get(
            `/beneficiaryRegistration/findPensionerMasterDetailsByApplicationNo/${formData.applicationNo}`
          );

          const beneficiaryData = beneficiaryResponse.data;
          console.log('beneficiary data kya hai', beneficiaryData.dateOfBirth);

          const formattedDateOfBirth = dayjs(beneficiaryData.dateOfBirth, 'DD-MM-YYYY');
          console.log('beneficiaryData', beneficiaryData);
          const { dataType, ...restFormData } = formData;

          beneficiaryData.dataType = dataType;
          ...
          setFormData((prev) => ({
            ...
            applicantName: beneficiaryAadharData.nameAsPerUid,
            gender:beneficiaryAadharData.gender 
          }));

          try {
            if (beneficiaryData.schemeCode === 'NFBS') {
              const nfbsResponse = await axiosInstance.get(
                `/beneficiaryRegistration/findNfbsMasterDetailsByApplicationNo/${formData.applicationNo}`
              );
              ...
              if (nfbsData.dateOfDeath) {
                ...
                nfbsData.dateOfDeath = dateAdapter.date(dayjs(nfbsData.dateOfDeath));
              }
              console.log('nfbsData.dateOfDeath1', nfbsData.dateOfDeath);
              setSelectedDistrictNfbs(nfbsData.districtId1);
              setSelectedAreaNfbs(nfbsData.ruralUrbanArea1);
              setSelectedSubDistrictNfbs(nfbsData.subDistrictMunicipalAreaId1);
              setSelectedGramPanchayatWardNfbs(nfbsData.gramPanchayatWardId1);
              setSelectedVillageNfbs(nfbsData.villageId1);
              setSelectedHabitationNfbs(nfbsData.habitationId1);

              setNfbsFormData(nfbsData);
            }
          } catch (error) {
            console.error('Error fetching data NFBS:', error);
          }
          ...
          setSelectedDistrict(beneficiaryData.districtId);
          setSelectedArea(beneficiaryData.ruralUrbanArea);
          setSelectedSubDistrict(beneficiaryData.subDistrictMunicipalAreaId);
          setSelectedGramPanchayatWard(beneficiaryData.gramPanchayatWardId);
          setSelectedVillage(beneficiaryData.villageId);
          setSelectedHabitation(beneficiaryData.habitationId);
          //console.log('Setting applicantName from Aadhar:', beneficiaryAadharData.nameAsPerUid);
  ...
  //console.log('dateOfDeath AFTER USEEFFECT', nfbsFormData.dateOfDeath);

  const handleSelectState = (selectedStatetId) => {
    setFormData({ stateId: selectedStatetId });
  };

  const handleSelectStateNfbs = (selectedStatetId) => {
    setNfbsFormData({ stateId1: selectedStatetId });
  };
  ...
  const handleSelectDistrict = (selectedDistrictId) => {
    ...
    setFormData({
      ...
      ruralUrbanArea: userLevel === 4 ? areaCode : '',
  ...
  const handleSelectDistrictNfbs = (selectedDistrictId) => {
    setSelectedDistrictNfbs(selectedDistrictId);
    setNfbsFormData({
      ...nfbsFormData,
      districtId1: selectedDistrictId,
      ruralUrbanArea1: userLevel === 4 ? areaCode : '',
    ...
    setSelectedAreaNfbs('');
    setSelectedSubDistrictNfbs('');
    setSelectedGramPanchayatWardNfbs('');
    setSelectedVillageNfbs('');
    setSelectedHabitationNfbs('');
  ...
  const handleSelectAreaNfbs = (selectedAreaId) => {
    setSelectedAreaNfbs(selectedAreaId);
    setNfbsFormData({
      ...nfbsFormData,
  ...
  const handleSelectSubDistrictNfbs = (selectedSubDistrictId) => {
    setSelectedSubDistrictNfbs(selectedSubDistrictId);
    setNfbsFormData({
      ...nfbsFormData,
  ...
  const handleGramPanchayatChange = (selectedGramPanchayatWardId) => {
    ...
    setFormData({ ...formData, gramPanchayatWardId: selectedGramPanchayatWardId, villageId: '', habitationId: '' });
  ...
  const handleGramPanchayatChangeNfbs = (selectedGramPanchayatWardId) => {
    setSelectedGramPanchayatWardNfbs(selectedGramPanchayatWardId);
    setNfbsFormData({ ...nfbsFormData, gramPanchayatWardId1: selectedGramPanchayatWardId, villageId1: '', habitationId1: '' });
  ...
  const handleSelectedVillage = (selectedVillageId) => {
    ...
    setFormData({ ...formData, villageId: selectedVillageId, habitationId: '' });
    setSelectedHabitation('');
    setFormErrors((prevErrors) => {
      const updatedErrors = { ...prevErrors };
      delete updatedErrors.villageId;
  ...
  const handleSelectedVillageNfbs = (selectedVillageId) => {
    setSelectedVillageNfbs(selectedVillageId);
    setNfbsFormData({ ...nfbsFormData, villageId1: selectedVillageId, habitationId1: '' });
    setSelectedHabitationNfbs('');
  };

  const handleSelectedHabitation = (selectedHabitationId) => {
    setSelectedHabitation(selectedHabitationId);
    setFormData({ ...formData, habitationId: selectedHabitationId });
    console.log(selectedHabitation);
  ...
  const handleSelectedHabitationNfbs = (selectedHabitationId) => {
    setSelectedHabitationNfbs(selectedHabitationId);
    setNfbsFormData({ ...nfbsFormData, habitationId1: selectedHabitationId });
    console.log(selectedHabitationNfbs);
  };
  ...
  const handleChange = (field, value) => {
    if (field === 'dateOfBirth') {
      ...
      setFormData({ ...formData, age: age, dateOfBirth: value });
    } else if (field === 'age') {
      const currentDate = dayjs();
      ...
      setFormData({ ...formData, age: value, dateOfBirth: birthDate });
    } else {
  ...
  // console.log('dateOfDeath INPUT', nfbsFormData.dateOfDeath);
  const handleChangeNfbs = (field, value) => {
    if (field === 'dateOfDeath') {
      // console.log('dateOfDeath OUTPUT IF ', value);
      // console.log('dateOfDeath OUTPUT IF ', nfbsFormData.dateOfDeath);
      /* const dateOnly = dayjs(value);
      const formattedDate = dateOnly.format('YYYY-MM-DD'); */
      const dateAdapter = new AdapterDayjs();
      const parsedDate = dayjs(value);
      const dateOfDeath = dateAdapter.date(parsedDate);
    ...
    console.log('dateOfDeath OUTPUT LAST ', value);
    console.log('dateOfDeath OUTPUT LAST ', nfbsFormData.dateOfDeath);
    validateForm(field, value, 1, 0);
  };
  //console.log('dateOfDeath OUTPUT ', nfbsFormData.dateOfDeath);

  function parseDate(paramDate) {
    if (paramDate) {
      const dateAdapter = new AdapterDayjs();
      try {
        const parsedDate = dayjs(paramDate, 'YYYY-MM-DD');
        if (parsedDate.isValid()) {
          return dateAdapter.date(parsedDate);
        } else {
          console.error('Invalid date format:', authorizeCertIssueDate);
          return paramDate;
        }
      } catch (error) {
      ...
      <div>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.dataType ? 'error' : ''}>
              <InputLabel id="demo-simple-select-label">
                Pensioner Type&nbsp;<span style={{ color: 'red' }}>*</span>
              </InputLabel>
              <Select
                ...
                error={formErrors.dataType ? true : false}
              >
              ...
              {formErrors.dataType && (
                <FormHelperText>
                  <Typography style={{ color: 'red', fontSize: '0.8rem' }}>{formErrors.dataType}</Typography>
                </FormHelperText>
              )}
            </FormControl>
          </Grid>
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.gender ? 'error' : ''}>
              ...
              <Select
                ...
                label="Gender"
                name="gender"
                value={beneficiaryAadharData.gender || ''}
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.districtId ? 'error' : ''}>
              <DistrictList
                ...
                defaultSelectedDistrict={selectedDistrict}
                isMandatory={true}
              />
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.ruralUrbanArea ? 'error' : ''}>
              <AreaList selectedArea={selectedArea} onSelectArea={handleSelectArea} isMandatory={true} />
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.gramPanchayatWardId ? 'error' : ''}>
              <GramPanchayatList
                defaultSelectGramPanchayatWardId={selectedGramPanchayatWard}
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.villageId ? 'error' : ''}>
              <VillageList
                ...
                selectedGramPanchayatId={selectedGramPanchayatWard}
                setSelectedVillage={formData.villageId}
                onSelectVillage={handleSelectedVillage}
                isDisabled={formData.ruralUrbanArea === 'U' ? true : false}
                isMandatory={formData.ruralUrbanArea === 'R' || formData.ruralUrbanArea === undefined ? true : false}
              />
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.habitationId ? 'error' : ''}>
              <HabitationList
                ...
                defaultSelectedHabitation={selectedHabitation}
      ...
      {getSchemeCode === 'NFBS' && (
        <>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth>
                <StateList onSelectState={handleSelectStateNfbs} isMandatory={true} />
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth className={formErrors.districtId1 ? 'error' : ''}>
                <DistrictList
                  ...
                  defaultSelectedDistrict={selectedDistrictNfbs}
                  isMandatory={true}
                />
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth className={formErrors.ruralUrbanArea1 ? 'error' : ''}>
                <AreaList selectedArea={selectedAreaNfbs} onSelectArea={handleSelectAreaNfbs} isMandatory={true} />
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth className={formErrors.subDistrictMunicipalAreaId1 ? 'error' : ''}>
                <SubDistrictList
                  defaultSelectedSubDistrict={selectedSubDistrictNfbs}
                  selectedAreaId={selectedAreaNfbs}
                  selectedDistrictId={selectedDistrictNfbs}
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth className={formErrors.gramPanchayatWardId1 ? 'error' : ''}>
                <GramPanchayatList
                  defaultSelectGramPanchayatWardId={selectedGramPanchayatWardNfbs}
                  selectedSubDistrictMunicipalAreaId={nfbsFormData.subDistrictMunicipalAreaId1}
                  //setSelectedGramPanchayatNfbs={setSelectedGramPanchayatWardNfbs}
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth>
                <VillageList
                  defaultSelectVillageId={selectedVillageNfbs}
                  selectedGramPanchayatId={selectedGramPanchayatWardNfbs}
                  setSelectedVillage={nfbsFormData.villageId1}
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth>
                <HabitationList
                  selectedVillageId={selectedVillageNfbs}
                  defaultSelectedHabitation={selectedHabitationNfbs}
              ...
              <FormControl fullWidth className={formErrors.dateOfDeath ? 'error' : ''}>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DatePicker
                    ...
                    slotProps={{ textField: { fullWidth: true } }}
                    variant="outlined"
                    value={parseDate(nfbsFormData.dateOfDeath)}
