...
const BeneficiaryBankDetails = ({
  ...
  console.log('-----formData.applicationNo BANK -----', formData.applicationNo);
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (formData.applicationNo) {
          const response = await axiosInstance.get(
            `/beneficiaryRegistration/findPensionerBankDetailsByApplicationNo/${formData.applicationNo}`
          );
          const beneficiaryData = response.data;
          console.log('beneficiaryData BANK', beneficiaryData);
  ...
  }, [formData.applicationNo, setFormData]);
  useEffect(() => {
    const dataTypeFromLocalStorage = localStorage.getItem('dataType');
  ...
  /* useEffect(() => {
    const applicationNoFromLocalStorage1 = localStorage.getItem('applicationNumberPersonal');
    if (applicationNoFromLocalStorage1) {
      handleChange('applicationNo', applicationNoFromLocalStorage1);
    }
  }, []); */
          ...
          <Grid item xs={12} sm={4}>
            <TextField
              ...
              InputProps={{
                ...
                endAdornment: (
                  <>
                    {formData.ifscCode && !ifscCodeValid && (
