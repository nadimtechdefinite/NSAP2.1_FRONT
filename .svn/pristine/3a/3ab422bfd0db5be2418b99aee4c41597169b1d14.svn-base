...
import { Grid, Button, FormControl, Backdrop, CircularProgress, Divider, Chip, FormHelperText, Typography } from '@mui/material';
...
export default function ApproveAccountBankDetailsUpdation() {
  ...
  const columns = [
...

    {
      ...
      headerName: 'Bank Account No Old',
      flex: 1,
      editable: false,
      disableColumnMenu: true
    },
    {
      field: 'ifscCode',
      headerName: 'Ifsc Code Old',
      flex: 1,
      editable: false,
      disableColumnMenu: true
    },
    {
      field: 'bankPoAccountNoNew',
      headerName: 'Bank Account No New',
      flex: 1,
      editable: false,
      disableColumnMenu: true
    },
  ...
  const fetchData = async () => {
    ...
    if (isFormValid) {
      try {
        ...
        const getUrl = `/approve-bank-account-updation/getBankAccountConsentUpdation`;
        setLoading(true);
  ...
  const rows = getAllDisData.map((item, index) => ({
    ...
    bankPoAccountNoNew: item.bankPoAccountNoNew,
    ifscCodeNew: item.ifscCodeNew,
    dys: item.dys + ' Days'
  }));
  ...
  const handleButtonClick1 = async (e, rowId, action, status) => {
    try {
      ...
      try {
        var apiUrl = null;
        if (action === 'true') {
          apiUrl = '/approve-bank-account-updation/approveBankAccountUpdationData';
          setLoading(true);
        } else {
          apiUrl = '/approve-bank-account-updation/rjctBankAccountUpdationData';
          setLoading(true);
        }
  ...
  return (
    <div>
      <MainCard title="Approve Bank Account Updation">
        {!!snackbar && (
