...
const BeneficiaryBPLDetails = ({ formData, setFormData, nfbsFormData, setNfbsFormData, formErrors, validateForm, setFormErrors }) => {
  let dataTypeFromLocalStorage = localStorage.getItem('dataType');
  const [fileDownload, setFileDownload] = useState(true);
  const userinfoHeader = getUserInfo();
  const apiBaseUrl = config.API_BASE_URL;

  useEffect(() => {
    // Set the retrieved dataType in the form data
    if (dataTypeFromLocalStorage) {
      setFormData((prevData) => ({
        ...prevData,
        dataType: dataTypeFromLocalStorage
      }));
    }
  }, [formData.bplMemberId]);
  ...
  console.log('-----formData.applicationNo BPL -----', formData.applicationNo);
  useEffect(() => {
      ...
      try {
        ...
        if (formData.applicationNo) {
          const response = await axiosInstance.get(
            `/beneficiaryRegistration/findPensionerOtherDetailsByApplicationNo/${formData.applicationNo}`
          );
          const beneficiaryData = response.data;

          console.log('beneficiaryData OTHER', beneficiaryData);
          ...
          try {
            if (beneficiaryData.schemeCode === 'NFBS') {
              ...
              //console.log('nfbsData.authorizeCertIssueDate', nfbsData.authorizeCertIssueDate);
              console.log('nfbsData.authorizeCertIssueDate', nfbsData.authorizeCertIssueDate);
              ...
              console.log('nfbsData.authorizeCertIssueDate1', nfbsData.authorizeCertIssueDate);
              setNfbsFormData((prevData) => ({
                ...prevData,
                ...nfbsData,
                authorizeCertIssueDate: nfbsData.authorizeCertIssueDate,
          ...
          console.log('application date input', beneficiaryData.applicationDate);
          ...
          console.log('application date parsed', beneficiaryData.applicationDate);
  ...
  console.log('nfbsFormData.authorizeCertIssueDate', nfbsFormData.authorizeCertIssueDate);
  //console.log('nfbsFormData.authorizeCertIssueDateParsed', parseDate(nfbsFormData.authorizeCertIssueDate));
  ...
  const handleChangeNfbs = (field, value) => {
    if (field === 'authorizeCertIssueDate') {
      ...
      const authorizeCertIssueDate = dateAdapter.date(parsedDate);
      //console.log('formattedDate', formattedDate);
      setNfbsFormData((prevData) => ({
        ...prevData,
        [field]: authorizeCertIssueDate
      }));
    } else {
      setNfbsFormData((prevData) => ({
        ...prevData,
  ...
  /*  useEffect(() => {
    let applicationNoFromLocalStorage3;

    if (localStorage.getItem('applicationNumberUid') !== 'undefined') {
      applicationNoFromLocalStorage3 = localStorage.getItem('applicationNumberUid');
      handleChange('applicationNo', applicationNoFromLocalStorage3);
    } else if (localStorage.getItem('applicationNumberBank') !== 'undefined') {
      applicationNoFromLocalStorage3 = localStorage.getItem('applicationNumberBank');
      handleChange('applicationNo', applicationNoFromLocalStorage3);
    } else {
      applicationNoFromLocalStorage3 = localStorage.getItem('applicationNumberPersonal');
      handleChange('applicationNo', applicationNoFromLocalStorage3);
    }
  }, []); */
  ...
  const [currentDate] = useState(dayjs()); // Add this line to get the current date
  useEffect(() => {
    // Set the retrieved dataType in the form data
  ...
  const handleFileChange = (e, fileType) => {
    ...
    if (file) {
      ...
      // Clear error message for the fileType field
      const newFormErrors = { ...formErrors };
      delete newFormErrors[fileType]; // Clear the error message for the fileType field
      newFormData[fileType] = file;
      setFormData(newFormData);
      setFormErrors(newFormErrors);
      console.log('date check ', newFormData.bankCertIssueDate);
    }
  ...
  const handleDownload = (filename) => {
    ...
    fetch(`${apiBaseUrl}/beneficiaryRegistration/downloadFile/${filename}/${formData.applicationNo}`, {
      method: 'GET',
      headers: {
  ...
  const handleNfbsDownload = (filename) => {
    const token = getAuthToken();
    fetch(`${apiBaseUrl}/beneficiaryRegistration/downloadNfbsFile/${filename}/${formData.applicationNo}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <div>
                    ...
                    <FormControl fullWidth className={formErrors.authorizeCertIssueDate ? 'error' : ''}>
                      <LocalizationProvider dateAdapter={AdapterDayjs}>
                        <DatePicker
                          ...
                          variant="outlined"
                          value={parseDate(nfbsFormData.authorizeCertIssueDate)}
        ...
        {dataTypeFromLocalStorage === 'EXISTING' && (
          <>
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.ageCertIssueAuth ? 'error' : ''}>
              <TextField
                ...
                onChange={(e) => handleChange('ageCertIssueAuth', e.target.value)}
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.incCertIssueAuth ? 'error' : ''}>
              <TextField
                ...
                onChange={(e) => handleChange('incCertIssueAuth', e.target.value)}
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.resCertIssueAuth ? 'error' : ''}>
              <TextField
                ...
                onChange={(e) => handleChange('resCertIssueAuth', e.target.value)}
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth className={formErrors.disCertIssueAuth ? 'error' : ''}>
                  <TextField
                    ...
                    onChange={(e) => handleChange('disCertIssueAuth', e.target.value)}
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth className={formErrors.deathCertIssueAuth ? 'error' : ''}>
                  <TextField
                    ...
                    onChange={(e) => handleChange('deathCertIssueAuth', e.target.value)}
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth className={formErrors.deathFile ? 'error' : ''}>
                  <TextField
                    ...
                    //{schemeCode === 'IGNWPS' ? "Upload Husband's Death Certificate" : " Upload Deseased's Death Certificate "}
                    name="deathFile"
                    variant="outlined"
                    fullWidth
                    InputProps={{
          ...
          <Grid item xs={12} sm={4}>
            <FormControl fullWidth className={formErrors.bankCertIssueAuth ? 'error' : ''}>
              <TextField
                ...
                onChange={(e) => handleChange('bankCertIssueAuth', e.target.value)}
