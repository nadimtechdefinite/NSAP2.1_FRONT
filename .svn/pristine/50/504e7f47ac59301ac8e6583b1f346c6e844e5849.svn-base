...
const SocialAuditReportForStatesAndAtn = () => {
  ...
  const columnsReport = [
    ...
    { field: 'stname', headerName: 'State', align: 'left', headerAlign: 'left', width: 330 },
    {
      field: 'socialFile',
  ...
  const getSocialAuditReportSummary = async () => {
    try {
      ...
      const response = await axiosInstance.post(
        `report/showSocialAuditReportOfStatesAndAtn/${stateCode}/${finYear}/${captcha}/${captchaToken}`
      );
  ...
  return (
    <>
      <MainCard>
        <h3 style={{ fontSize: '2rem', color: 'black', fontWeight: 'normal' }}>Social Audit Report conducted by states and ATN</h3>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={3}>
            <FormControl fullWidth>
              ...
              >
                {/* Adding "All States" option */}
                <MenuItem value="ALL">All States</MenuItem>
                {/* Rendering other states */}
              ...
              <DataGrid
                ...
                slotProps={{
                  toolbar: {
                    ...
                    csvOptions: { fileName: 'SOCIAL_AUDIT_REPORT_FOR_STATES_AND_ATN' }
                  }
                }}
                rows={dataSummary}
...
export default SocialAuditReportForStatesAndAtn;
