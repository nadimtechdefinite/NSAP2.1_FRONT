...
function Preview() {
  ...
  const storedData = localStorage.getItem('applicationNumberOther');
  ...
  const getGithubData = async () => {
    try {
      const registrationDetailsResponse = await axiosInstance.get(`/beneficiaryRegistration/getAllRegistrationDetails/${storedData}`);
      const allBpl = registrationDetailsResponse.data;
      console.log('preview data', allBpl);

      if (schemeCode === 'NFBS') {
        const nfbsMasterDetailsResponse = await axiosInstance.get(
          `/beneficiaryRegistration/findNfbsMasterDetailsByApplicationNo/${storedData}`
        );
  ...
  const handleDownload = (filename) => {
    ...
    fetch(`${apiBaseUrl}/beneficiaryRegistration/downloadFile/${filename}/${storedData}`, {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    const fetchProfilePhoto = async (filename) => {
      try {
        ...
        const response = await fetch(`${apiBaseUrl}/beneficiaryRegistration/downloadFile/${filename}/${storedData}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
  ...
  const handleNfbsDownload = (filename) => {
    const token = getAuthToken();
    fetch(`${apiBaseUrl}/beneficiaryRegistration/downloadNfbsFile/${filename}/${storedData}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    <ThemeProvider theme={theme}>
      <div className="myText">
        ...
        <div className="preview-content">
          <div className="preview-section">
            ...
            <Grid container spacing={0}>
              <Grid item xs={12} sm={3}>
                <p>
                  <strong>Application No: {storedData}</strong>
                </p>
              </Grid>
              <Grid item xs={12} sm={3}>
                <p>
              ...
              <Grid item xs={12} sm={4}>
                ...
                <Dialog open={showAadhaar} onClose={handleClose} fullWidth maxWidth="sm">
                  ...
                  <DialogContent dividers>
                    {!aadhaarData ? (
                      <Typography variant="body2">Loading Aadhaar data...</Typography>
                    ) : (
          ...
          {schemeCode === 'NFBS' && (
            <div className="preview-section">
              <h2>Personal Details of Decesaed</h2>
              <Grid container spacing={0}>
                <Grid item xs={12} sm={3}>
