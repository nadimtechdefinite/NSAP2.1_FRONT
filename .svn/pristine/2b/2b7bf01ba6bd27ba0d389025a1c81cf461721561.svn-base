...
const SocialCategory = () => {
  ...
  const findReportSummary = async () => {
    try {
      const getUrl = `${apiBaseUrl}/home/findCountSocialCategoryWisePensioners`;
      const response = await axios.get(getUrl);
      setCountData(response.data[0]);
    } catch (error) {
      console.error('Error fetching find Count Social Category Wise Pensioners Summary', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <MainCard title="Social Category">
      {loading ? (
        <Box
          sx={{
            display: 'flex',
