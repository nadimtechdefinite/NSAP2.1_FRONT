...
function ApproveMarkNSAPBeneficiarySubDis() {
  ...
  const handleChangeVer = (value,rowId) => {
      ...
      const getLabelData = async () => {
        try {
            const findAllLabelData = '/approve-mark-nsap-beneficiary/getApproveMarkNSAPBeneficiaryForSubDistrict/' + localStorage.getItem('districtId');
            setLoading(true);
      ...
      const handleDataForSubdistrict = (gpCode) => {
        //alert(' View  -- '+gpCode)
        localStorage.setItem("gpId", gpCode);
        localStorage.setItem('districtId',localStorage.getItem('districtId'));
        navigate('/verification/approveMarkNSAPBeneficiaryGP');
      };

      const backItem = () => {
        localStorage.setItem('districtId', '');
        navigate('/verification/approveMarkNSAPBeneficiary');
      };

      async function downloadDataForSubDistrict(distrcitCode,subDistrcitCode) {
        try {
          var gp="0";
          const postUrl = '/approve-mark-nsap-beneficiary/downloadDataForDistrict/'+distrcitCode+"/"+subDistrcitCode+"/"+gp;
          setLoading(true);
  ...
  useEffect(() => {
    getLabelData();getLabelFirstData();getDistrictNameData();
  }, []);
    ...
    },
   {
    ...
    renderCell: (params) => (
        <a title='Click' href={`#`} 
          ...
          onClick={(e) => {
          e.preventDefault(); handleDataForSubdistrict(params.row.subDistrictCode); }}>
          {params.value}
        </a>
      ),
  },
        ...
        if (params.value > 0) {
          return (
            ...
            href={`#`} 
              ...
              onClick={(e) => {
                e.preventDefault(); downloadDataForSubDistrict(params.row.districtCode,params.row.subDistrictCode); }}>
    ...
    return React.useCallback(
      (updatedRow ) =>
        new Promise((resolve, reject) => {
          setTimeout(() => {
            if (updatedRow.subDistrictCode?.trim() === '') {
              reject(new Error("Error: District Name can't be empty."));
            } else {
              resolve({ ...updatedRow , subDistrictCode: updatedRow .subDistrictCode?.toUpperCase() });
            }
  ...
  const mutateRow = useFakeMutation();
    ...
    const processRowUpdate = React.useCallback(
      async (updatedRow) => {
        try {
          ...
          setAllDistrict((prevRows) => {
            const rowIndex = prevRows.findIndex((row) => row.subDistrictCode === updatedRowData.subDistrictCode);
    ...
    const handleFormSubmit = async (e) => {
      ...
      const selectedData = getAllDistrict.filter(
        (row) => selectedRows.indexOf(row.subDistrictCode) !== -1
      );
      ...
      try {
        ...
        const apiUrl = '/approve-mark-nsap-beneficiary/saveApproveMarkNSAPBeneficiarySubDistrict'; 
        setLoading(true);
...
<div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap'   }}>
  <kbd className="responsive-kbd" style={{fontSize:'13px'}}>District Name: {districtName}</kbd>
     ...
     {!!snackbar && (
      ...
      <DataGrid checkboxSelection disableRowSelectionOnClick getRowId={(row) => row.subDistrictCode} 
...
export default ApproveMarkNSAPBeneficiarySubDis