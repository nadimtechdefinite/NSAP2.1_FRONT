...
const SchemeMasterListByStateCode = ({ selectedStateCode, onSelectScheme, isMendatory, defaultSelectedScheme }) => {
  ...
  useEffect(() => {
    ...
    const fetchSchemes = async () => {
      try {
        const getUrl = `/common/findAllSchemeByStateCode/${selectedStateCode}`;
        const response = await axiosInstance.get(getUrl);
        const data = await response.data;
        const allCentreSchemes = {
          schemeCode: 'ALL CENTRE SCHEMES',
          schemeShortName: 'All Centre Schemes'
        };
...
export default SchemeMasterListByStateCode;
