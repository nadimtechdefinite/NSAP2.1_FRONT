...
import { Grid, Button, FormControl, Divider, Chip, Backdrop, CircularProgress, FormHelperText, Typography } from '@mui/material';
...
export default function GenerateSanctionOrderNew() {
  ...
  const options = {
    sanctionOrderNo: 'Application No'
  };
  ...
  const fetchData = async () => {
    ...
    if (isFormValid) {
      try {
        ...
        const getUrl = `/sanction-order-master/getPendingSanctionOrderNoRecordNew`;
        setLoading(true);
  ...
  const rows = getAllDisData.map((item) => ({
    ...
    veRlev1Date: item.veRlev1Date,
    pendingDays: item.pendingDays == null ? 'NA' : item.pendingDays.toString().replace(/\.0+$/, ''),
  ...
  function handleButtonForView(id, appno) {
    //alert("-- "+appno);
    localStorage.setItem('appnoSanction', appno);
    navigate('/sanction/viewsanction');
  }
